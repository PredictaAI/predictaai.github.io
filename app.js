...
async function fetchPredictions(pair, date) {
    try {
        ...
        const path = `/news/${formattedPair}/${formattedDate}/predictions.yaml`;
        ...
        return yamlData.predictions.map(prediction => ({
            ...prediction,
...
function openModal(prediction) {
    modalTitle.textContent = prediction.title;
...
async function renderTiles() {
    ...
    try {
        const predictions = await fetchPredictions(selectedPair, selectedDate);
        
        tileGrid.innerHTML = '';
        
        if (predictions.length === 0) {
            tileGrid.innerHTML = '<div class="no-data">No predictions available for this date</div>';
            return;
        }
